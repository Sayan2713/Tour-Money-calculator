...
const axios = require('axios'); // <--- RESTORED THIS
...
// --- EMAIL CONFIGURATION ---
// Using 'gmail' service is fine for localhost testing
  ...
  try {
    ...
    if (user) {
      if (user.isVerified) {
        ...
        // User exists but is NOT verified. Delete old to re-register.
    ...
    const newUser = new User({
      ...
      mobile: mobile || '',
      dob: dob || null,
      isVerified: false, // <--- Important: User is NOT active yet
      otp,
      otpExpires
    });
    ...
    // SUCCESS: Do NOT send token yet. Ask client to verify.
...
// ############################################################################
// 2. SIGNUP STEP 2: Verify OTP & Complete Registration (RESTORED)
// ############################################################################
...
// ############################################################################
// 5. FORGOT PASSWORD ROUTES (These were fine, kept as is)
// ############################################################################

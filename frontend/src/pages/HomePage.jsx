            ...
            return (
              <div key={p._id} className="flex items-center space-x-1">
                ...
                <input
                  type="number"
                  placeholder={
                    editedExpense.splitType === "PERCENT" ? "%" : "₹"
                  }
                  className="w-16 p-1 border rounded text-xs"
                  value={val}
                  onChange={(e) =>
                    onSplitValueChange(p.name, "value", e.target.value)
                  }
                />
                {editedExpense.splitType === "EXACT" && (
                  <input
                    ...
                    value={item}
                    onChange={(e) =>
                      onSplitValueChange(p.name, "item", e.target.value)
                    }
                  />
                )}
              </div>
    ...
    return (
      <tr className="bg-yellow-50 border-b">
        ...
        <td className="px-4 py-3 align-top text-center space-y-2">
          <button
            onClick={onSave}
            className="block w-full text-xs bg-green-500 hover:bg-green-600 text-white px-2 py-1 rounded"
          >
            Save
          </button>
          <button
            onClick={onCancel}
            className="block w-full text-xs bg-gray-500 hover:bg-gray-600 text-white px-2 py-1 rounded"
          >
            Cancel
          </button>
        </td>
  ...
  return (
    <tr className="hover:bg-gray-50 border-b">
      ...
      <td className="px-4 py-3 text-sm text-gray-700 max-w-xs">
        {/* <div className="truncate" title={(expense.sharedBy || []).join(", ")}>
          {(expense.sharedBy || []).join(", ")}
        </div> */}
        <div
          className="truncate"
          title={(expense.sharedBy || [])
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    setNewExpense((prev) => ({
      splitType: "EQUAL",
      splitDetails: {},
      sharedBy: prev.sharedBy || participants.map((p) => p.name),
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      api
        .get(`/participants/${selectedTrip._id}`)
        .then((res) => setParticipants(res.data))
        .catch(() => {});
      api
        .get(`/expenses/${selectedTrip._id}`)
  ...
  useEffect(() => {
    if (!editingExpenseId && !isSplitTouched) {
      setNewExpense((prev) => ({
        ...prev,
        sharedBy: prev.sharedBy?.length
          ? prev.sharedBy
          : participants.map((p) => p.name),
      }));
    }
  }, [participants, editingExpenseId, isSplitTouched]);
  ...
  const handleAddTrip = (e) => {
    ...
    api
      .post("/trips/add", { name: newTripName })
      .then(() => {
        setNewTripName("");
        fetchTrips();
      ...
      .catch((err) =>
        showAlert(err.response?.data?.Error || "Error adding trip", "error")
      );
  };
  ...
  const handleDeleteTrip = (tripId) => {
    ...
    api
      ...
      .catch((err) =>
        showAlert(err.response?.data?.Error || "Error deleting trip", "error")
      );
  };
  ...
  const handleSubmitExpense = (e) => {
    ...
    if (!newExpense.title || isNaN(totalAmt) || totalAmt <= 0)
      return showAlert("Invalid details", "warning");

    const categoryToSend =
      newExpense.category === "Other"
        ? newExpense.otherCategory?.trim() || "Other"
        : newExpense.category;
    ...
    if (newExpense.splitType === "EQUAL") {
      ...
      if (payload.sharedBy.length === 0)
        return showAlert("Select at least one person", "warning");
    } else {
      let details = [];
      let runningTotal = 0;
      participants.forEach((p) => {
        ...
        if (val > 0) {
          let calculatedAmount =
            newExpense.splitType === "PERCENT" ? (val / 100) * totalAmt : val;
          runningTotal +=
            newExpense.splitType === "PERCENT" ? val : calculatedAmount;
          details.push({
            name: p.name,
            value: val,
      ...
      if (details.length === 0)
        return showAlert("Enter split details", "warning");
      if (
        newExpense.splitType === "PERCENT" &&
        Math.abs(runningTotal - 100) > 0.1
      )
        return showAlert(`Total % is ${runningTotal}, must be 100%`, "warning");
      if (
        newExpense.splitType === "EXACT" &&
        Math.abs(runningTotal - totalAmt) > 0.1
      )
        return showAlert(
          `Total split amount ₹${runningTotal} does not match expense ₹${totalAmt}`,
          "warning"
        );
  ...
  const handleEditExpense = (exp) => {
    ...
    let detailsMap = {};
    if (exp.splitDetails && Array.isArray(exp.splitDetails))
      exp.splitDetails.forEach(
        (d) => (detailsMap[d.name] = { value: d.value, item: d.item })
      );
    if (
      exp.splitDetails &&
      !Array.isArray(exp.splitDetails) &&
      typeof exp.splitDetails === "object"
    ) {
    ...
    setEditedExpense({
      ...
      otherCategory:
        exp.category &&
        !["Food", "Transport", "Lodging", "Misc"].includes(exp.category)
          ? exp.category
          : "",
    });
  };
  ...
  const handleSaveEdit = () => {
    ...
    const categoryToSend =
      editedExpense.category === "Other"
        ? editedExpense.otherCategory?.trim() || "Other"
    ...
    api
      .put(`/expenses/update/${editingExpenseId}`, payload)
      .then(() => {
  ...
  const renderAlert = () => {
    ...
    return (
      <div
        className={`fixed top-20 right-4 z-50 p-4 rounded-lg shadow-lg text-white font-semibold ${
    ...
    return (
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 p-4 border rounded-lg bg-gray-50 max-h-32 overflow-y-auto">
        {participants.length === 0 && (
          <span className="text-gray-500 italic">Add participants first.</span>
        )}
        {participants.map((p) => (
          <label
            key={p._id}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 relative">
            ...
            <form onSubmit={handleSendInvite}>
              ...
              />
              {inviteStatus === "error" && (
                <p className="text-red-500 mb-2">{inviteMsg}</p>
              )}
              {inviteStatus === "success" && (
                <p className="text-green-500 mb-2">{inviteMsg}</p>
              )}
              <button
                type="submit"
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          >
            ...
            <form onSubmit={handleSubmitExpense} className="space-y-3">
                  ...
                  {participants.map((p) => (
                    ...
                    >
                      ...
                      {newExpense.splitType === "EQUAL" && (
                        <input
                          type="checkbox"
                          checked={newExpense.sharedBy?.includes(p.name)}
                          onChange={() => {
                            setIsSplitTouched(true);
                            const cur = newExpense.sharedBy || [];
                            setNewExpense({
                              ...newExpense,
                              sharedBy: cur.includes(p.name)
                      ...
                      {newExpense.splitType === "PERCENT" && (
                        <div className="flex items-center gap-1">
                          <input
                            ...
                            className="w-16 p-1 border rounded text-xs"
                            value={
                              newExpense.splitDetails?.[p.name]?.value || ""
                            }
                            onChange={(e) =>
                      ...
                      {newExpense.splitType === "EXACT" && (
                        <div className="flex gap-1 flex-1 justify-end">
                          ...
                          <input
                            ...
                            className="w-20 p-1 border rounded text-xs"
                            value={
                              newExpense.splitDetails?.[p.name]?.item || ""
                            }
                            onChange={(e) =>
                              handleSplitValueChange(
                                p.name,
              ...
              <button
                type="submit"
                className="w-full bg-indigo-600 text-white font-bold py-2 rounded"
              >
                Add Expense
              </button>
            </form>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4 mb-4 text-center">
              ...
              <div className="bg-purple-50 p-2 rounded">
                <div className="text-gray-600 text-xs">Per Person</div>
                <div className="text-xl font-bold text-purple-800">
                  ₹
                  {(
                    settlement.totalSpent /
                    Math.max(1, settlement.balances.length)
                  ).toFixed(2)}
                </div>
              </div>
            </div>
            <div className="space-y-2 mb-6">
              {!settlement.netPayments.length && (
              ...
              {settlement.netPayments.map((p, i) => (
                <div
                  key={i}
                  className="flex justify-between bg-red-50 p-2 rounded text-sm"
                >
                  <span>
                    {p.from} <span className="text-red-500 font-bold">→</span>{" "}
                    {p.to}
                  </span>
              ...
              <tbody>
                ...
                {expenses.map((e) => (
                  <ExpenseRow
                    ...
                    onEditChange={(f, v) =>
                      setEditedExpense((prev) => ({ ...prev, [f]: v }))
                    }
